		...
		function activateMouseHandle() { 
			...
			$(document)
				.bind('mousemove', function(event) {
					...
					if (!flipInProgress) {
						if (dragging) {
							mousePosition = {left: event.pageX - $(holder).position().left, top: event.pageY - options.pageHeightOffset + 160};
						}
					}
				})
				.bind('touchmove', function(event) {
					...
					if (!flipInProgress) {
						if (dragging && event.originalEvent != null && event.originalEvent.touches.length == 1) {
							...
							mousePosition = {left: touch.screenX - $(holder).position().left, top: touch.screenY - options.pageHeightOffset + 160};
						}
					}
				})
